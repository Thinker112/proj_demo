...
public class ReadFewFrame {
    ...
    static void save_frame(AVFrame pFrame, int width, int height, int f_idx) throws IOException {
        // Open file
        String szFilename = String.format("frame_%d.ppm", f_idx);
        OutputStream pFile = Files.newOutputStream(Paths.get(szFilename));
    ...
    public static void main(String[] args) throws Exception {
        ...
        // I don‘t know but without this function, sws_getContext does not work
        ...
        while (av_read_frame(fmt_ctx, pkt) >= 0) {
            if (pkt.stream_index() == v_stream_idx) {//专门处理视频流
